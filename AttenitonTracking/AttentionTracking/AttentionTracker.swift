...
final class AttentionTracker {
    ...
    private let batchSubject: PassthroughSubject<AttentionTracking.Batch, Never> = .init()
    // send all batches into this subject
    private let outputSubject: PassthroughSubject<[AttentionTracking.Output], Never> = .init()
    ...
    private func setupSubscriptions() {
        ...
        var helperDict: AttentionTracking.HelperDict = .init()

        batchSubject
            .receive(on: queue) // receive event and continue on the serial queue
            .compactMap { batch -> [AttentionTracking.Output]? in
                var outputs: [AttentionTracking.Output] = .init()
                // processing new batch, the result will be in outputs array
                AttentionTracking.processBatch(
                    batch: batch,
    ...
    func trackVisibleIds(_ visibleIds: Set<Int>, date: Date = .init()) {
        ...
        let batch: AttentionTracking.Batch = .init(visibleIds: visibleIds, date: date)
        batchSubject.send(batch)
    }

    // returning output from Combine subject as a Concurrency's output sequence
    var outputSequence: any AsyncSequence<[AttentionTracking.Output], Never> {
